...
export default function PointHistoryPagination() {
  const { point_info, userPointHistory } = accountStore()
  console.log('point_info : ' , point_info)



  const currentPage = (point_info?.offset || 0) + 1
  const totalPages = (point_info?.last_offset || 0) + 1
  const totalItems = point_info?.item_list_total || 0  // 서버에 존재하는 총 히스토리 개수
  const historySets = point_info?.logs || []  // 현재 offset에 대한 히스토리 리스트

  
  // 페이지 변경 핸들러 (서버에 새 데이터 요청 필요)
  const handlePageChange = async (newPage: number) => {
    await userPointHistory(newPage - 1)
  }
  ...
  // 교환 상태에 따른 한글 표시
  const getExchangeStatusText = (isExchange: boolean) => {
    return isExchange ? '완료' : '대기중'
  }
              ...
              >
                ...
                <td style={{ padding: '8px 10px' }}>
                  <div style={{ color: '#6b7280', fontSize: '12px' }}>
                    {history.tx_time && history.tx_time !== '1970-01-01T00:00:00Z' ? formatDate(history.tx_time) : '-'}
                  </div>
                </td>
                ...
                <td style={{ padding: '8px 10px', textAlign: 'right' }}>
                  <div style={{ fontWeight: '600', color: '#1f2937', fontSize: '13px' }}>
                    {formatNumber(wei2eth(history.amount), 2)} BABI
                  </div>
                </td>
                ...
                <td style={{ padding: '8px 10px' }}>
                  <span style={{
                    ...
                    fontSize: '12px',
                    fontWeight: '500',
                    backgroundColor: history.is_exchange ? '#dcfce7' : '#ffedd5',
                    color: history.is_exchange ? '#166534' : '#9a3412'
                  }}>
                    {getExchangeStatusText(history.is_exchange)}
                  </span>
                </td>
                ...
                <td style={{ padding: '8px 10px' }}>
                  <div style={{ color: '#6b7280', fontSize: '12px' }}>
                    {history.exchange_time && history.exchange_time !== '1970-01-01T00:00:00Z' ? formatDate(history.exchange_time) : '-'}
                  </div>
                </td>
                ...
                <td style={{ padding: '8px 10px' }}>
                  <a 
                    href={`${getChainInfoByKey('bsc').explorer_tx}${history.tx_hash}`}
                    target="_blank"

...
export default function SalePage() {
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if(amount > 10) {
      ...
      if (amount < minAmount) {
        setErrorMessage(`최소 구매 금액은 ${minAmount} BNB입니다.`)
        return
      }
    }
  ...
  const handlePurchase = async() => {
    ...
    if (amount > parseFloat(wei2eth(user_info.bsc.coin_balance).toString())) {
  ...
  function getMinDepositAmount(): number {
    if(sale_info && sale_info.bsc) {
      return wei2eth(sale_info.bsc.sale_contract.bnb.min_deposit)
    }
    return 0;
  }
  function getMaxDepositAmount(): number {
    ...
    const maxDeposit = wei2eth(sale_info?.bsc?.sale_contract?.bnb?.max_deposit || '0')
    const tokenPerBNB = wei2eth(sale_info?.bsc?.sale_contract?.bnb?.token_per_bnb || '0')
  ...
  function getButtonState(): ButtonState {
    const isPaused = sale_info?.bsc?.sale_contract?.bnb?.paused || false;
    if(isPaused) {
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <div style={{ 
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ 
                      ...
                      {formatNumber(wei2eth(user_info.bsc.coin_balance), 2)} BNB
                    </div>
                  </div>
                </div>
                ...
                <div style={{ 
                  ...
                  <div style={{ 
                    ...
                    <span>1 BNB당 BABI 수량</span>
                    <span style={{ fontWeight: '600' }}>{formatNumber(Number(wei2eth(sale_info?.bsc?.sale_contract?.bnb?.token_per_bnb || '0')), 2)} BABI</span>
                  </div>
                  <div style={{ 
                    display: 'flex', 
                    justifyContent: 'space-between', 
                    ...
                    <span>최소 구매 금액</span>
                    <span style={{ fontWeight: '600' }}>{formatNumber(getMinDepositAmount(), 2)} BNB</span>
                  </div>
                  <div style={{ 
                    display: 'flex', 
                    justifyContent: 'space-between', 
                    ...
                    <span>최대 구매 금액</span>
                    <span style={{ fontWeight: '600' }}>{formatNumber(getMaxDepositAmount(), 2)} BNB</span>
                  </div>
                </div>

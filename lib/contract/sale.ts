...
export function makeDataDepositBNB(amount: string): string {
    const iface = new ethers.utils.Interface(['function depositBNB(uint256 amount) external']);
    return iface.encodeFunctionData('depositBNB', [amount]);
}
